...
import Tidal_History, { extractPatternName } from './history';
...
export default {
  ...
  toggle() {
    ...
    editor.onDidChange(() => {
      ...
      let patternStr = extractPatternName(paragraphTypedIn)[0];
  ...
  createBox (lineNum, patternNum, isGraph) {
    ...
    // Sometimes the marker gets wonky if we add/remove from the start of it.
    // This is a kind of janky way to fix that.
    marker.onDidChange(e=>{
      if (e.newHeadBufferPosition.column === 0) return;
      // In the weird zone -- try just resetting the buffer range!
      // Hope this doesn't have some other weird edge cases lol :)
      let row = e.newHeadBufferPosition.row;
      let column = 0;
      marker.setBufferRange({start: {row, column}, end: {row, column}});
    });
