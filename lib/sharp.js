...
export default {
  ...
  toggle() {
    ...
    if (numBoots === 0) {
      ...
      // saving mechanism
      atom.commands.add('atom-workspace', 'sharp:dumpHistory', (e) => {
        this.dumpHistory(e);
      });
  ...
  dumpHistory(event) {
    ...
    if (editor) {
      ...
      newFileName = filePath.substring(0, filePath.length - 6) + "_history_" + dateTimeString + ".tidal";
    ...
    atom.confirm({
      ...
      buttons: {
        Save: function(value) {
          // write to file
          fs.writeFile(newFileName, strToWrite, function(err) {
            if (err) throw err;
