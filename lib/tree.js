...
export function Tree(data, { // data is either tabular (array of objects) or hierarchy (nested objects)
  ...
  node.append("circle")
      ...
      .attr("fill", d => d.children ? fill : fill)
      .attr("id", function (d) { return IDPREPEND + d.data.name; })
      ...
      .on("click", function (evt, d) {
        ...
        for(let i = 0; i < patternLen; i++){
          ...
          d3.select(idstr).style("fill","gray");
        }
        //d3.select("circle").style("fill","gray");
        //select the node in the graph
        let idstr = '#' + IDPREPEND + colorNodeIndex;
        d3.select(idstr).style("fill","red");
