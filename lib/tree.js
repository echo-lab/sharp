...
export function Tree(data, { // data is either tabular (array of objects) or hierarchy (nested objects)
  ...
  const dy = scaleXPos
    ? Math.max(30, width / (Math.max(data.length, 8) + padding))
  ...
  node.append("circle")
      ...
      .on("click", function (evt, d) {
        ...
        logger && logger.log({
          action: "CLICK",
          patternName: this.pattern_history.patternName,
          patternNum: this.pattern_history.patternNum,
          nodeIdx: d.data.name,
        });
        ...
        let r = getCurrentBlockRange(editor);
      ...
      .on("mouseover", function (_, d) {
        ...
        let prev_text_range = getCurrentBlockRange(editor);
        ...
        //get the text that relates to the box and replace it. Note: this resets the scroll for some reason.
