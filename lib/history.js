...
const KEEP_TREES_UNIQUE = true;
var start_time = 0; //ms

export default class Node {
  constructor(strLine, timeCreated){
...
export default class Pattern_History {

  constructor (patternNum) {
  ...
  setSelectedNode (node) {
    for(let i = 0; i < this.pattern_history.length; i++) {
      ...
      else if(this.pattern_history[i].matches(node)){
...
export default class Tidal_History
{
  ...
  constructor () {
    ...
    for (let i = 0; i < NUM_PATTERNS; i++) {
      this.pattern_histories[i] = new Pattern_History(i);
    }
  }
  ...
  reportChange (pattern_num) {
    arr = this.pattern_histories[pattern_num].getPatternHistory();
    if(arr.length >= 2){
  ...
  addToHistory (currLine) {
    ...
    let pattern_num;
    let pattern_str = currLine.substring(0, currLine.indexOf("$")).trim();

    //if it starts with d, remove that char and read the numbers
    if(pattern_str.startsWith('d')) {
      pattern_str = pattern_str.substring(1);
    }
    //otherwise find first number in string
    else {
      pattern_str.replace( /(^.+)(\w\d+\w)(.+$)/i,'$2');
    }
    ...
    console.log(this.pattern_histories[pattern_num]);
    ...
    console.log("Created at time: " + this.pattern_histories[pattern_num].getSelectedNode().getTimeCreated());
